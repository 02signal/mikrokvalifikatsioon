...
- **Brand follow-ups** (from the brand-manual + logo work, `docs/brand-manual.md`): (1) apply the
  animated `Logo` at the konto save-confirmation moment ("Sinu valikud on hoitud") - the strongest
  brand moment per the manual, deliberately not done yet to avoid over-applying; (2) optionally
  generate a raster favicon/og from `public/logo/mark.svg` so the SVG mark is the single source;
  (3) consider swapping the EN homepage `<img src="/mk-logo.png">` (`src/pages/en/index.astro`) for
  the `Logo` component for consistency.
